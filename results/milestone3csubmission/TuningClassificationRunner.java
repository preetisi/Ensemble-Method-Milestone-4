...
public class TuningClassificationRunner extends ClassificationRunner {
	...
	protected ClassifierResult runClassifier(Instances trainingData, Instances testingData,
		...
		if (PARAMETER_TUNING_OPTIONS.containsKey(classifierClass)) {
			...
			return runParameterDecorate(trainingData, testingData);
		} else {
	...
	private ClassifierResult runParameterDecorate(Instances trainingData, Instances testingData)
			throws Exception {
		DecorateParameterTuner tuner = new DecorateParameterTuner();
	...
	public static void main(String[] args) throws Exception {
		if (args.length < 3) {
			System.err
					.println("Usage: TuningClassificationRunner <data dir> <output dir> <'old' or 'new'>");
		} else {
			String[] datasets;
			...
			new TuningClassificationRunner(args[0], args[1], datasets, CLASSIFIERS)
					.runClassifiers();
		}
	}
}
